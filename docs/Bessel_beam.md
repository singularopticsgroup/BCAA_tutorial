...
Examples of the generated amplitude maps are below. First one shows how manipulation of axicon angels affects final hologram. A) \(refractive index = 1.1\), \(axicon angle = 0.2\deg\), \(m = 0\); B) \(refractive index = 1.1\), \(axicon angle = 0.5\deg\), \(m = 0\)
<p align="center">
